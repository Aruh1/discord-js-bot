...
module.exports = {
    ...
    async messageRun(message, args) {
        ...
        if (sub === "start") {
            if (!args[1])
                return message.safeReply("Incorrect usage! Please provide a channel to start the giveaway in");
...
async function runModalSetup({ member, channel, guild }, targetCh) {
    ...
    const modal = await btnInteraction
        .awaitModalSubmit({
            time: 1 * 60 * 1000,
            filter: m =>
                m.customId === "giveaway-modalSetup" && m.member.id === member.id && m.message.id === sentMsg.id
        })
        .catch(ex => {});
